...
def mikrotik_fwd(src, out, remove={"google.com"}):
    ...
    with open(f"{out}-mikrotik-fwd.lst", "w") as file:
        for name in domains:
            if name.startswith("."):
                file.write(
                    f"/ip dns static add name=*.{name[1:]} type=FWD address-list=allow-domains forward-to=localhost\n"
                )
            else:
                file.write(
                    f"/ip dns static add name={name} type=FWD address-list=allow-domains match-subdomain=yes forward-to=localhost\n"
                )
...
def generate_srs_for_categories(
    directories, output_json_directory="JSON", compiled_output_directory="SRS"
):
        ...
        for filename in os.listdir(directory):
            ...
            if os.path.isfile(file_path):
                ...
                output_file_path = os.path.join(
                    output_json_directory, f"{os.path.splitext(filename)[0]}.json"
                )
    ...
    for filename in os.listdir(output_json_directory):
        if filename.endswith(".json"):
            ...
            srs_file_path = os.path.join(
                compiled_output_directory, f"{os.path.splitext(filename)[0]}.srs"
            )
            try:
                subprocess.run(
                    [
...
def generate_srs_subnets(
    input_file, output_json_directory="JSON", compiled_output_directory="SRS"
):
    os.makedirs(output_json_directory, exist_ok=True)
    os.makedirs(compiled_output_directory, exist_ok=True)
    ...
    data = {"version": 3, "rules": [{"ip_cidr": subnets}]}
    ...
    try:
        subprocess.run(
            ["sing-box", "rule-set", "compile", output_file_path, "-o", srs_file_path],
            check=True,
        )
        print(f"Compiled .srs file: {srs_file_path}")
    except subprocess.CalledProcessError as e:
...
def generate_srs_combined(
    input_subnets_file,
    input_domains_file,
    output_json_directory="JSON",
    compiled_output_directory="SRS",
):
    os.makedirs(output_json_directory, exist_ok=True)
    os.makedirs(compiled_output_directory, exist_ok=True)
    ...
    if input_subnets_file == "Subnets/IPv4/discord.lst":
        data = {
            ...
            "rules": [
                ...
                {"network": ["udp"], "ip_cidr": subnets, "port_range": ["50000:65535"]},
            ],
        }
    elif (
        input_subnets_file == "Subnets/IPv4/telegram.lst"
        and input_domains_file == "voice_messengers"
    ):
        data = {
            "version": 3,
            "rules": [
        ...
        data = {"version": 3, "rules": [{"domain_suffix": domains, "ip_cidr": subnets}]}
...
def generate_dat_domains(
    data_path="geosite_data", output_name="geosite.dat", output_directory="DAT"
):
    os.makedirs(output_directory, exist_ok=True)
